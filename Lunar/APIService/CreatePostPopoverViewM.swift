...
struct CreatePostPopoverViewM: View {
    ...
    Section {
      Button {
        ...
        if submittable {
          ...
          let createPostParameters = CreatePostParameters(name: postName, communityId: communityID, url: postURL, body: postBody, languageId: 0, auth: JWT().getJWTFromKeychain(actorID: activeAccount.actorID) ?? "")
          
          PostSender().createPost(parameters: createPostParameters) { response, postID in
            if response == "success" {
